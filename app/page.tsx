...
import { 
  ...
  Plane, Calendar, CloudSun, FileText
} from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans selection:bg-red-100 selection:text-red-900">
      ...
      {isProfileOpen && (
        <div className="fixed inset-0 z-40 bg-transparent cursor-default" onClick={() => setIsProfileOpen(false)}></div>
      )}
      ...
      <header className="absolute top-0 left-0 right-0 z-50 h-[90px] px-6 md:px-12 flex justify-between items-center border-b border-white/10 backdrop-blur-[2px]">
        <div className="flex items-center gap-4">
           <img src="/thy_logo.png" alt="THY" className="h-10 w-auto object-contain brightness-0 invert opacity-90" onError={(e) => {e.currentTarget.style.display='none'}} />
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="relative z-50">
               <button onClick={() => setIsProfileOpen(!isProfileOpen)} className={`p-2.5 rounded-full text-white transition-all backdrop-blur-md border border-white/10 ${isProfileOpen ? 'bg-red-600 border-red-500 ring-4 ring-red-900/30' : 'bg-white/10 hover:bg-white/20'}`}>
                  <User size={20} />
               </button>
      ...
      <div className="relative w-full h-[75vh] bg-slate-900 overflow-hidden">
         <img src="/thy_gok.jpg" alt="Hero" className="absolute inset-0 w-full h-full object-cover opacity-60 scale-105 animate-[pulse_20s_ease-in-out_infinite]" />
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-stretch">
               ...
               <div className="w-full lg:w-1/3 animate-in slide-in-from-bottom duration-1000 delay-200">
                  <div className="h-full bg-white rounded-2xl shadow-2xl shadow-slate-900/10 p-8 border-t-8 border-red-600 relative overflow-hidden group flex flex-col justify-between min-h-[320px]">
                     ...
                     <div className="mt-8">
                       <button onClick={() => setIsModalOpen(true)} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold flex items-center justify-between px-6 hover:bg-emerald-600 transition-colors group/btn shadow-lg">
                          İŞLEM YAP
               ...
               <div className="w-full lg:w-2/3 bg-white rounded-2xl shadow-2xl shadow-slate-900/10 border-t-8 border-slate-200 p-8 animate-in slide-in-from-bottom duration-1000 delay-300 flex flex-col">
                  ...
                  {shortcutList.map((text, idx) => (
                        <a key={idx} href="#" className="flex items-center gap-3 group cursor-pointer p-2 rounded-lg hover:bg-slate-50 transition-colors">
                           ...
                           <span className="text-[11px] font-bold text-slate-500 group-hover:text-slate-900 transition-colors truncate">{text}</span>
                        </a>
                  ))}
                  </div>
               </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl relative">
            ...
            <div className="p-6 bg-slate-50 grid gap-4">
              ...
              <Link href="/taleplerim" className="flex items-center gap-4 p-5 bg-white border border-slate-200 shadow-sm hover:shadow-lg hover:border-indigo-500 hover:-translate-y-1 transition-all rounded-xl group">
                <div className="bg-indigo-50 p-3 rounded-full text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors"><FileText size={24} /></div>
                <div><h4 className="font-bold text-slate-800 text-sm">Taleplerim</h4><p className="text-xs text-slate-500 mt-1">Geçmiş ve aktif kayıtlarım</p></div>
